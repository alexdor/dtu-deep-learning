...
def decode_batch(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        decoded_batch = decode_batch_output(
            out.decoded_tokens, vocab, batch.oov_dict
        )
...
def eval_batch(
    ...
    *,
    ...
    decoded_batch, out = decode_batch(
        batch,
        model,
        vocab,
        criterion=criterion,
...
def eval_batch_output(
    tgt_tensor_or_tokens: Union[torch.Tensor, List[List[str]]],
    vocab: Vocab,
    oov_dict: OOVDict,
    *pred_tensors: torch.Tensor
) -> List[Dict[str, float]]:
    """
  ...
  """
    decoded_batch = [
        decode_batch_output(pred_tensor, vocab, oov_dict)
        for pred_tensor in pred_tensors
    ]
    if isinstance(tgt_tensor_or_tokens, torch.Tensor):
        gold_summaries = decode_batch_output(
            tgt_tensor_or_tokens, vocab, oov_dict
        )
    else:
...
def eval_bs_batch(
    ...
    *,
    ...
    best_only=True,
    details: bool = True
) -> Tuple[Optional[List[Dict[str, float]]], Optional[str]]:
    """
  ...
  """
    ...
    if details:
        ...
        file_content = None
    if (
        batch.examples[0].tgt is not None
    ):  # run ROUGE if gold standard summary exists
        gold_summaries = [
            batch.examples[0].tgt for _ in range(len(decoded_batch))
        ]
        scores = rouge(gold_summaries, decoded_batch)
        if details:
            file_content += "\n\n\n[Reference Summary]\n" + format_tokens(
                batch.examples[0].tgt
            )
            file_content += (
...
def eval_bs(test_set: Dataset, vocab: Vocab, model: Seq2Seq, params: Params):
    ...
    for i in prog_bar:
        batch = next(test_gen)
        scores, file_content = eval_bs_batch(
            batch,
            model,
            vocab,
