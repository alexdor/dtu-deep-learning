...
from config import data_path, vocab_path, vocab_size

finished_path = os.path.join(data_path, "finished")
unfinished_path = os.path.join(data_path, "unfinished")
chunk_path = os.path.join(data_path, "chunked")
...
def creating_finished_data():
    ...
    write_to_bin(
        os.path.join(unfinished_path, "train.art.shuf.txt"),
        os.path.join(unfinished_path, "train.abs.shuf.txt"),
        train_bin_path,
        vocab_counter,
    )
    write_to_bin(
        os.path.join(unfinished_path, "valid.art.shuf.txt"),
...
def chunk_file(set_name, chunks_dir, bin_file):
    ...
    while not finished:
        chunk_fname = os.path.join(
            chunks_dir, "%s_%04d.bin" % (set_name, chunk)
        )  # new chunk
...
if __name__ == "__main__":
    ...
    samples = random.sample(
        set(bin_chunks[:-1]), 2
    )  # Exclude last bin file; contains only 9k sentences
    valid_chunk, test_chunk = samples[0], samples[1]
    shutil.copyfile(
        os.path.join(chunk_path, "main_valid", valid_chunk),
        os.path.join(chunk_path, "valid", "valid_00.bin"),
    )
    shutil.copyfile(
        os.path.join(chunk_path, "main_valid", test_chunk),
