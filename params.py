        ...
        for arg_text in cmd_args:
            if arg_name is None:
                assert arg_text.startswith(
                    "--"
                )  # the arg name has to start with "--"
                arg_name = arg_text[2:]
            else:
                ...
                if arg_text.lower() == "none":
                    ...
                    if (
                        type(arg_type) is not type
                    ):  # support only Optional[T], where T is a basic type
                        assert arg_type.__origin__ is Union
                        arg_types = [
                            t for t in arg_type.__args__ if t is not type(None)
                        ]
                ...
                print(
                    "Hyper-parameter %s = %s (was %s)"
                    % (arg_name, arg_new_value, arg_curr_value)
                )
                arg_name = None
        if arg_name is not None:
            print(
                "Warning: Argument %s lacks a value and is ignored." % arg_name
            )
