...
class EncoderRNN(nn.Module):
    def __init__(
        self, embed_size, hidden_size, bidi=True, *, rnn_drop: float = 0
    ):
        super(EncoderRNN, self).__init__()
        self.hidden_size = hidden_size
        self.num_directions = 2 if bidi else 1
        self.gru = nn.GRU(
            embed_size, hidden_size, bidirectional=bidi, dropout=rnn_drop
        )
    ...
    def __init__(
        ...
        *,
        ...
        if enc_attn:
            ...
            self.enc_bilinear = nn.Bilinear(
                self.hidden_size, enc_hidden_size, 1
            )
    ...
    """
        ...
        if self.enc_attn or self.pointer:
            ...
            if self.enc_attn:
                ...
                enc_context = torch.bmm(
                    encoder_states.permute(1, 2, 0), enc_attn
                )
                combined[
                    :, offset : offset + enc_total_size
        ...
        if self.dec_attn:
            if decoder_states is not None and len(decoder_states) > 0:
                ...
                dec_context = torch.bmm(
                    decoder_states.permute(1, 2, 0), dec_attn
                )
                combined[
        ...
        if self.pointer:
            ...
            prob_ptr = sigmoid(self.ptr(combined))  # (batch size, 1)
            prob_gen = 1 - prob_ptr
...
class Seq2Seq(nn.Module):
    ...
    """
        ...
        if vocab.embeddings is not None:
            ...
            if (
                params.embed_size is not None
                and self.embed_size != params.embed_size
            ):
                print(
    ...
    """
        ...
        if forcing_ratio == 1:
            ...
            if partial_forcing:
                use_teacher_forcing = (
                    None  # decide later individually in each step
                )
            else:
        ...
        if visualize:
            r.enc_attn_weights = torch.zeros(
                target_length, batch_size, input_length
            )
            if self.pointer:
                r.ptr_probs = torch.zeros(target_length, batch_size)

        decoder_input = torch.tensor(
            [self.vocab.SOS] * batch_size, device=DEVICE
        )
        ...
        for di in range(target_length):
            ...
            if self.enc_attn_cover or (criterion and self.cover_loss > 0):
                if (
                    coverage_vector is not None
                    and criterion
                    and self.cover_loss > 0
                ):
                    coverage_loss = (
    ...
    """
        ...
        if max_out_len is None:
            max_out_len = (
                self.max_dec_steps - 1
            )  # max_out_len doesn't count EOS
        ...
        while (
            ...
            decoder_input = torch.tensor(
                [h.tokens[-1] for h in hypos], device=DEVICE
            )
            ...
            for nh in new_hypos:
                ...
                if nh.tokens[-1] == self.vocab.EOS:  # a complete hypothesis
                    ...
                    length == max_out_len
                    and len(new_incomplete_results) < beam_size
                ):
        ...
        if (
            ...
            results = (
                backup_results  # that are "truncated" at the end to max_out_len
            )
